 ...
 */
inline bool prop_decompose_recompose_2d(size_t M0, size_t M1, size_t x0,
                                        size_t x1) {
  if (!(M0 > 0))
    return true;
  if (!(M1 > 0))
 ...
 */
inline bool prop_coalesce_preserves_function(size_t s0, size_t d0, size_t s1,
                                             size_t d1, size_t x0, size_t x1) {
  if (!(s0 > 0))
    return true;
  if (!(s1 > 0))
    return true;
  if (!(d1 == s0 * d0))
    return true; // packed merge condition
  if (!(x0 < s0))
    return true;
  if (!(x1 < s1))
    return true;
 ...
 */
inline bool prop_predication_thm_1(size_t i0, size_t i1, size_t i2, size_t N0,
                                   size_t N2) {
  if (!(N2 > 0))
    return true;
  if (!(i0 < N0))
