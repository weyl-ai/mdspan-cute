...
TEST_CASE("mapping parity: 2D static layout", "[mapping]") {
  ...
  exts_t exts =
      mdspan_cute::detail::make_extents_from_shape<exts_t>(cute::shape(cl));
...
TEST_CASE("mapping parity: 1D dynamic", "[mapping]") {
  ...
  exts_t exts =
      mdspan_cute::detail::make_extents_from_shape<exts_t>(cute::shape(cl));
  mdspan_cute::layout_cute<CL>::template mapping<exts_t> m(
      cl); // uses layout->extents ctor
...
TEST_CASE("swizzled 2D mapping parity", "[swizzle]") {
  ...
  auto swz = cute::composition(mdspan_cute::swizzle::sw32{},
                               base); // simple row swizzle
...
TEST_CASE("mapping parity holds for dynamic rank-1", "[property][mapping]") {
  rc::prop(
      "mapping parity holds for dynamic rank-1", [](const std::size_t n0_) {
        ...
        using E = mdspan_cute::detail::cute_to_extents_t<std::size_t, S>;
        E exts =
            mdspan_cute::detail::make_extents_from_shape<E>(cute::shape(cl));
...
TEST_CASE("mapping parity holds for dynamic rank-2", "[property][mapping]") {
  rc::prop("mapping parity holds for dynamic rank-2", [](std::size_t m_,
                                                         std::size_t n_) {
    ...
    RC_ASSERT(mapp.required_span_size() ==
              static_cast<typename E::index_type>(cute::cosize(cl)));
    for_each_coord_safely(exts, [&](std::size_t i, std::size_t j) {
      RC_ASSERT(mapp(i, j) == static_cast<typename E::index_type>(
                                  cl(cute::make_tuple(i, j))));
    });
  });
}
...
TEST_CASE("mapping parity holds for dynamic rank-3", "[property][mapping]") {
  rc::prop("mapping parity holds for dynamic rank-3",
           [](std::size_t a_, std::size_t b_, std::size_t c_) {
             ...
             using S = mdspan_cute::cute_shape_t<CL>;
             using E = mdspan_cute::detail::cute_to_extents_t<std::size_t, S>;
             E exts = mdspan_cute::detail::make_extents_from_shape<E>(
                 cute::shape(cl));
             mdspan_cute::layout_cute<CL>::template mapping<E> mapp(exts, cl);
...
static_assert(ExtentsFromFlat1::static_extent(2) ==
...
static_assert(ExtentsFromFlat2::static_extent(2) ==
              std::experimental::dynamic_extent);
...
static_assert(DynExtents::static_extent(0) ==
              std::experimental::dynamic_extent);
static_assert(DynExtents::static_extent(1) == 8);
static_assert(DynExtents::static_extent(2) ==
              std::experimental::dynamic_extent);
