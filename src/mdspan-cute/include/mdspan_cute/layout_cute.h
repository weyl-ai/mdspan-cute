...
// Helper to get extent value: static for cute::Int<N>, dynamic for integral
// types Use std::integral_constant to force immediate evaluation in template
// contexts
template <typename T, typename = void>
struct cute_extent_value_or_dynamic
    : std::integral_constant<std::size_t, std::experimental::dynamic_extent> {};

template <typename T>
struct cute_extent_value_or_dynamic<
    T, std::enable_if_t<cute_extent_is_static_v<std::remove_cvref_t<T>>>>
    : std::integral_constant<std::size_t, cute_static_extent_value<
...
using shape_flatten_t =
    typename shape_flatten<std::remove_cvref_t<Shape>>::type;
...
struct cute_to_extents<IndexType, T> {
  using type =
      std::experimental::extents<IndexType, std::experimental::dynamic_extent>;
};
...
struct cute_to_extents<IndexType, cute::Int<N>> {
  using type =
      std::experimental::extents<IndexType,
                                 cute_static_extent_value<cute::Int<N>>::value>;
};

// Helper to build extent list
template <typename IndexType, std::size_t... Vals> struct make_extents_helper {
...
struct cute_to_extents<IndexType, cute::tuple<Ts...>> {
  using type = typename make_extents_helper<
      IndexType, cute_extent_value_or_dynamic<Ts>::value...>::type;
};

template <typename IndexType, typename CuteShape>
using cute_to_extents_t =
    typename cute_to_extents<IndexType, std::remove_cvref_t<CuteShape>>::type;
...
constexpr void fill_dyn_at(
  ...
  if constexpr (Extents::static_extent(I) ==
...
inline constexpr std::size_t cute_layout_flat_rank_v = cute::tuple_size<
    std::remove_cvref_t<shape_flatten_t<cute_shape_t<CuteLayout>>>>::value;
...
using cute_dmdspan =
    std::experimental::mdspan<T, std::experimental::dextents<std::size_t, Rank>,
                              layout_cute<CuteLayout>>;
