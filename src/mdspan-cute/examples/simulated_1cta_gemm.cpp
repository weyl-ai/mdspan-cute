...
using SmemLayoutA = decltype(composition(
    Swizzle<2, 3, 3>{}, make_layout(make_shape(Int<CTA_M>{}, Int<CTA_K>{}),
...
using SmemLayoutB = decltype(composition(
    Swizzle<2, 3, 3>{}, make_layout(make_shape(Int<CTA_K>{}, Int<CTA_N / 2>{}),
...
using SmemLayoutScaleA =
    decltype(make_layout(make_shape(Int<CTA_M>{}, Int<K_GROUPS>{}),
                         make_stride(Int<K_GROUPS>{}, Int<1>{})));

using SmemLayoutScaleB =
    decltype(make_layout(make_shape(Int<CTA_N>{}, Int<K_GROUPS>{}),
                         make_stride(Int<K_GROUPS>{}, Int<1>{})));
...
template <typename T, typename CuteLayout> struct TileView {
  using layout_policy = mdspan_cute::layout_cute<CuteLayout>;
  using extents_type =
      std::extents<int, size<0>(CuteLayout{}), size<1>(CuteLayout{})>;
...
void simulate_mainloop() {
  ...
  std::println("  1CTA style: {}×{}×{}, {} stages", CTA_M, CTA_N, CTA_K,
               STAGES);
  std::println(
      "═══════════════════════════════════════════════════════════════");
  std::println("");
  ...
  auto make_A_view = [&](int stage) {
    return ATile::mdspan_type(&smem.A[stage][0][0],
                              ATile::mapping_type(SmemLayoutA{}));
  };

  auto make_B_view = [&](int stage) {
    return BTile::mdspan_type(&smem.B[stage][0][0],
  ...
  std::println("  A:       [{}, {}] int8  (activations)", ATile::Rows,
               ATile::Cols);
  std::println("  B:       [{}, {}] uint8 (packed int4 weights)", BTile::Rows,
               BTile::Cols);
  ...
  for (int dm = 0; dm < 8; ++dm) {
    for (int dk = 0; dk < 8; ++dk) {
      ...
      float dequant = scale_a * scale_b * static_cast<float>(a_val) *
                      static_cast<float>(b_val);
      accum += dequant;
    }
  }

  std::println("  A[{}, {}] through A[{}, {}]", base_m, base_k, base_m + 7,
               base_k + 7);
  std::println("  B[{}, {}] through B[{}, {}]", base_k, base_n / 2, base_k + 7,
               (base_n + 7) / 2);
  ...
  for (int m = 0; m < CTA_M; ++m) {
    for (int k = 0; k < CTA_K; ++k) {
      ...
      int8_t actual = A[m, k];
      if (expected != actual) {
        std::println("  MISMATCH at [{}, {}]: expected {}, got {}", m, k,
                     expected, actual);
        all_correct = false;
      }
    }
  ...
  if (all_correct) {
    std::println("  ✓ All {}×{} = {} A tile elements correct", CTA_M, CTA_K,
                 CTA_M * CTA_K);
  }
  ...
  for (int k = 0; k < CTA_K; ++k) {
    for (int n = 0; n < CTA_N / 2; ++n) {
      ...
      if (expected != actual) {
        std::println("  MISMATCH at [{}, {}]: expected {}, got {}", k, n,
                     expected, actual);
        all_correct = false;
      }
    }
  ...
  if (all_correct) {
    std::println("  ✓ All {}×{} = {} B tile elements correct", CTA_K, CTA_N / 2,
                 CTA_K * CTA_N / 2);
  }
...
void analyze_bank_conflicts() {
  ...
  for (int m = 0; m < 32; ++m) {
    ...
    bank_histogram[bank]++;
    std::print(
        "  thread {:2d} → row {:2d}, col 0 → offset {:4d} → bank {:2d}\n", m, m,
        physical, bank);
  }
  ...
  std::println("");
  std::println("Max conflicts per bank: {} (ideal: 1, without swizzle: 32)",
               max_conflict);
}
