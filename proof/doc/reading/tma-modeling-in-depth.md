...
1. It is the outer output of a split of a TMA-protected IterDomain.
1. It is the output of a merge whose outer input is a TMA-protected IterDomain.
1. It is the output of a resize whose input is a TMA-protected IterDomain and `left_expand >= 0 && right_expand >= 0`.
1. It is the `X` output of a swizzle whose `X` input a TMA-protected IterDomain.
...
1. The box size is smaller than the tensor size on that dimension.
...
> **(eq 1)** There exists $i_s \\in \\mathbb{Z}$, $i_c \\in \\mathbb{Z}$, $i\_{t1} \\in \\mathbb{Z}$, $i\_{t2} \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>
> 1. $0 \\le i_s < e$
> 1. $0 \\le i_c < \\mathrm{ceilDiv}(S, B)$
> 1. $0 \\le i\_{t1}, i\_{t2} < \\mathrm{ceilDiv}(B, e)$
> 1. $0 \\le i_c \\times B + i\_{t1} \\times e + i_s < S$
> 1. $0 \\le i_c \\times B + i\_{t2} \\times e + i_s < S$
> 1. $0 \\le i\_{t1} \\times e + i_s < B$
> 1. $i\_{t2} \\times e + i_s \\ge B$

Using Theorem 1 in [Mathematical Logic](../math/logic.md), we simplify (eq 1) as:

> **(eq 2)** There exists $i_s \\in \\mathbb{Z}$, $i_c \\in \\mathbb{Z}$, $i\_{t1} \\in \\mathbb{Z}$, $i\_{t2} \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>
> 1. $0 \\le i_s < e$
> 1. $0 \\le i_c < \\mathrm{ceilDiv}(S, B)$
> 1. $0 \\le i\_{t1}, i\_{t2} < \\mathrm{ceilDiv}(B, e)$
> 1. $i_c \\times B + i\_{t1} \\times e + i_s < S$
> 1. $i_c \\times B + i\_{t2} \\times e + i_s < S$
> 1. $i\_{t1} \\times e + i_s < B$
> 1. $i\_{t2} \\times e + i_s \\ge B$

Rephrase (eq 2) as:

> **(eq 3)** There exists $i_s \\in \\mathbb{Z}$, $i\_{t1} \\in \\mathbb{Z}$, $i\_{t2} \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>
> 1. $0 \\le i_s < e$
> 1. $0 \\le i\_{t1}, i\_{t2} < \\mathrm{ceilDiv}(B, e)$
> 1. $i\_{t1} \\times e + i_s < B$
> 1. $i\_{t2} \\times e + i_s \\ge B$
> 1. There exists $i_c \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>    1. $0 \\le i_c < \\mathrm{ceilDiv}(S, B)$
>    1. $i_c < (S - (i\_{t1} \\times e + i_s)) \\div B$
>    1. $i_c < (S - (i\_{t2} \\times e + i_s)) \\div B$

The condition 5 in (eq 3) is true if and only if

> - $0 < \\mathrm{ceilDiv}(S, B)$, and
> - $0 < (S - (i\_{t1} \\times e + i_s)) \\div B$, and
> - $0 < (S - (i\_{t2} \\times e + i_s)) \\div B$

which can be simplified as

> - $i\_{t1} \\times e + i_s < S$, and
> - $i\_{t2} \\times e + i_s < S$

So (eq 3) can be simplified as:

> **(eq 4)** There exists $i_s \\in \\mathbb{Z}$, $i\_{t1} \\in \\mathbb{Z}$, $i\_{t2} \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>
> 1. $0 \\le i_s < e$
> 1. $0 \\le i\_{t1}, i\_{t2} < \\mathrm{ceilDiv}(B, e)$
> 1. $i\_{t1} \\times e + i_s < B$
> 1. $i\_{t2} \\times e + i_s \\ge B$
> 1. $i\_{t1} \\times e + i_s < S$
> 1. $i\_{t2} \\times e + i_s < S$

Rephrase (eq 4) as:

> **(eq 5)** There exists $i_s \\in \\mathbb{Z}$, $i\_{t2} \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>
> 1. $0 \\le i_s < e$
> 1. $0 \\le i\_{t2} < \\mathrm{ceilDiv}(B, e)$
> 1. $i\_{t2} \\times e + i_s \\ge B$
> 1. $i\_{t2} \\times e + i_s < S$
> 1. There exists $i\_{t1} \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>    1. $0 \\le i\_{t1} < \\mathrm{ceilDiv}(B, e)$
>    1. $i\_{t1} < (B - i_s) \\div e$
>    1. $i\_{t1} < (S - i_s) \\div e$
...
> **(eq 6)** There exists $i_s \\in \\mathbb{Z}$, $i\_{t2} \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>
> 1. $0 \\le i_s < e$
> 1. $0 \\le i\_{t2} < \\mathrm{ceilDiv}(B, e)$
> 1. $i\_{t2} \\times e + i_s \\ge B$
...
> **(eq 7)** There exists $i\_{t2} \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>
> 1. $0 \\le i\_{t2} < \\mathrm{ceilDiv}(B, e)$
> 1. There exists $i_s \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>    1. $0 \\le i_s < e$
>    1. $i_s < B$
>    1. $i_s < S$
>    1. $B - i\_{t2} \\times e \\le i_s$
>    1. $i_s < S - i\_{t2} \\times e$
...
> - $0 < B$, and
> - $0 < S$, and
> - $0 < S - i\_{t2} \\times e$, and
> - $B - i\_{t2} \\times e < e$, and
> - $B - i\_{t2} \\times e < B$, and
> - $B - i\_{t2} \\times e < S$, and
> - $B - i\_{t2} \\times e < S - i\_{t2} \\times e$
...
>
> 1. $B < S$
> 1. There exists $i\_{t2} \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>    1. $0 < i\_{t2} < \\mathrm{ceilDiv}(B, e)$
>    1. $i\_{t2} \\times e < S$
>    1. $B - e < i\_{t2} \\times e$
...
> 1. There exists $i\_{t2} \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>    1. $0 < i\_{t2}$
>    1. $B \\div e - 1 < i\_{t2} < \\mathrm{ceilDiv}(B, e)$
>    1. $i\_{t2} < S \\div e$

If $e$ divide $B$, then $\\mathrm{ceilDiv}(B, e) = B / e = B \\div e$,
then $B \\div e - 1 < i\_{t2} < \\mathrm{ceilDiv}(B, e)$ is not possible
...
> 1. There exists $i\_{t2} \\in \\mathbb{Z}$ that satisfies all of the following conditions:
>    1. $1 \\le i\_{t2}$
>    1. $\\mathrm{ceilDiv}(B, e) - 1 \\le i\_{t2} < \\mathrm{ceilDiv}(B, e)$
>    1. $i\_{t2} < S \\div e$

The condition (3.ii) in (eq 11) is achievable only if $i\_{t2} = \\mathrm{ceilDiv}(B, e) - 1$.
...
> 1. $B < S$
> 1. $e$ does not divide $B$
> 1. $\\mathrm{ceilDiv}(B, e) \\ge 2$
> 1. $S > e \\times (\\mathrm{ceilDiv}(B, e) - 1)$
...
1. **Level 2:** Some hole-creating IterDomain expressions' input IterDomains are not TMA-protected, but the condition of FTTC is false.
1. **Level 3:** The condition of FTTC is true.
